        ...
        <>
            <div className="success_icon mx-auto mt-1 ">
                <span className="icon_circle">
                    <SuccessCircle
                        stroke={`${responseType === 'success' ? '#27923E' : '#EE4034'}`}
                    />
                </span>

                <span
                    className={`${
                        responseType === 'error' ? `${'icon_' + responseType}` : 'icon_check'
                    }`}
                >
                    {responseType === 'success' ? <SuccessCheck /> : <Error />}
                </span>
            </div>
            <p
                className={`${
                    responseType === 'success' ? 'success' : 'modal_error'
                } message mx-auto`}
            >
                {responseType === 'error' ? 'Failed!' : 'Successful!'}
            </p>

            <p
                className={`${
                    responseType === 'success' ? 'success_info' : 'error_info'
                } description mx-auto`}
            >
                {responseTexts}
            </p>
