...
//   const { id } = useParams();    
  ...
  const paymentResponse = JSON.parse(localStorage.getItem("paymentResponse"))
    const { service, status, h, txRef, action } = paymentResponse;
    ...
    let download = () => {
    let url = `https://slipserver1.nimc.gov.ng/ijebu?h=${generateHash(jwt_data)}&p=1&type=${service === 1 ? "ninslip" : "pns"}`;
    ...
    return (
        <>
            <div className="">
                <h3 className="tab_page_title mx-auto">Print {service === 1 ? "Standard" : service === 2 ? "Premium" : ""} NIN Slip</h3>
                ...
                <div className="col-12 print_standard_slip d-flex justify-content-center px-4">
                    <ResponseCard action={action} status={status} serviceType={service} loading={loading} onclick={action === 'print' ? download : checkPaymentStatus} />
                </div>
            </div>
            {errorHandler && 
                <Modal
